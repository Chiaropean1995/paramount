...
export default function UpcomingProjects() {
    ...
    const handleDeleteProject = async (id) => {
        try {
            await axios.delete(`${url}/upcomingprojects/${id}`);
            fetchUpcomingProjects(); // Fetch updated bookings after deletion
        } catch (error) {
            console.error('Error deleting project:', error);
        }
    };

    const totalPages = Math.ceil(upcomingProjects.length / PAGE_SIZE);
    const startIndex = (currentPage - 1) * PAGE_SIZE;
    const endIndex = Math.min(startIndex + PAGE_SIZE, upcomingProjects.length);
    const currentProjects = upcomingProjects.slice(startIndex, endIndex);
                ...
                loading ? ( // Render spinner if loading
                    ...
                    <Row >
                        {filteredProjects.length === 0 ? (
                            <p className="text-center" style={{ color: 'red', fontStyle: 'italic', marginTop: '20px' }}>No upcoming projects found</p>
                        ) : (
                            <Row >
